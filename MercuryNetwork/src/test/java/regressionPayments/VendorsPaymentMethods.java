...
/**
 * <h1>Payments - Vendors Payment Methods</h1>
 * 
 * <p>.
 *
...
public class VendorsPaymentMethods extends TestSetup {

	/** The user. */
	private final String user = "PaymentMethodsAppraiser1";
...
* </ul>
	...
	public void addCreditCard() throws Exception{
		...
		vendors.addTestCreditCard(driver, creditCardNumber, false);
...
* </ul>
	...
	public void deleteCreditCard() throws Exception{
		...
		int numOfPaymentMethods = perform.getNumOfRowsInTableID(driver, VAccount.paymentMethods_table())-1;
...
* </ul>
	...
	public void verifyCanNotAddInvalidRoutingNumberOnACHAccount() throws Exception{
		...
		perform.type(driver,VAccount.routingNumber_txtbx(driver), "123456789");
		
		// Click out of the routing number field
		perform.click(driver,VAccount.accountNumber_txtbx(driver));
		
		// Wait for invalid text to update
		perform.waitForText(driver, VAccount.routingNumberValidation_txt(driver), "INVALID ROUTING NUMBER");
		
		// Verify INVALID ROUTING NUMBER text displays
		Assert.assertTrue(VAccount.routingNumberValidation_txt(driver).getText().contains("INVALID ROUTING NUMBER"), "The Invalid Routing Number text is incorrect");
		
		// Enter Account number
		perform.type(driver,VAccount.accountNumber_txtbx(driver), perform.randomNumbers(driver, 6));
		
		// Enter Confirm account number
		perform.type(driver,VAccount.confirmAccountNumber_txtbx(driver), perform.randomNumbers(driver, 6));
		...
		// Wait for OK button
		perform.waitForElementToBeClickable(driver, VAccount.ok_btn(), "id");
		
		// Verify valid routing number text
		Assert.assertTrue(VAccount.message_txt(driver).getText().contains("The following is required."), "The message text is incorrect");
...
* </ul>
	...
	public void addACHAccount() throws Exception{
		...
		vendors.addTestACHAccount(driver, accountNumber);
		
		// Go to Account
		vendors.goToAccount(driver);
		
		// Verify Name on account
		Assert.assertTrue(VAccount.paymentMethods_data(driver, "1", "3").getText().contains("Auto"), "Name on account is incorrect");
		Assert.assertTrue(VAccount.paymentMethods_data(driver, "1", "3").getText().contains("ACHUser"), "Name on account is incorrect");
		
		// Verify Account information
		String last4OfAccountNumber = accountNumber.substring(11,15);
		System.out.println("last4 = " + last4OfAccountNumber);
		Assert.assertTrue(VAccount.paymentMethods_data(driver, "1", "2").getText().contains("account ending in " + last4OfAccountNumber), "Account information is incorrect");
...
* </ul>
	...
	public void cannotAddMoreThan2PaymentMethods() throws Exception{
		...
		Assert.assertTrue(VAccount.dialog_txt(driver).getText().contains("Maximum reached"), "The Maximum reached dialog text is incorrect");
		Assert.assertTrue(VAccount.dialog_txt(driver).getText().contains("You may only store a maximum of two (2) payment methods."), "The Maximum reached dialog text is incorrect");
...
* </ul>
	...
	public void ensurePrimary() throws Exception{
		...
		vendors.deleteIndividualPaymentMethod(driver, "2");
		...
		vendors.addTestCreditCard(driver, creditCardNumber, true);
		...
		// Verify Account information
		Assert.assertTrue(VAccount.paymentMethods_data(driver, "1", "2").getText().contains("Active for fees (x4444)"), "The account information is incorrect");
		Assert.assertTrue(VAccount.paymentMethods_data(driver, "1", "5").getText().equals("Primary"), "This account should be the primary");
		
		// Verify second payment method is listed as the backup
		Assert.assertTrue(VAccount.paymentMethods_data(driver, "2", "5").getText().equals("Backup"), "This account should be the backup");
...
* </ul>
	...
	public void ensureBackup() throws Exception{
		...
		// Verify the new card is now the Primary
		// Verify Account information
		Assert.assertTrue(VAccount.paymentMethods_data(driver, "2", "2").getText().contains("Active for fees (x0005)"), "This account should be the backup");
		Assert.assertTrue(VAccount.paymentMethods_data(driver, "2", "5").getText().equals("Backup"), "This account should be the backup");
...
} // end the VendorsPaymentMethods class
