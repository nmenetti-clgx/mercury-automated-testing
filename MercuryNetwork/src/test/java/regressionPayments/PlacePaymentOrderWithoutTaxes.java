...
public class PlacePaymentOrderWithoutTaxes extends TestSetup {
	...
	public void placeNewPaymentOrder() throws Exception{
		...
		String receivable = setCombined;
		String received = "$0.00";
		String receivableDue = setCombined;
		String payable = setCombined;
		String payableDue = setCombined;
		String issueDate = "";
		...
		String amount = setCombined;
		received = "$0.00";
		String paid = "";
		String amountDue = setCombined;
		...
		secure.makePaymentFromXSiteUsingPaymentLink(driver, orderNumber, setOrderFee, setTechFee, setCombined, "Automation", "Test", "4111111111111111", "November", "2026", "73099", "automation@dntest.net");
		...
		String sqlStatement = "SELECT TransactionID FROM Merchants.dbo.Transactions WHERE ProductItemID = (SELECT ProductItemID FROM Mercury.dbo.ProductItems WHERE LoanID = "+orderNumberVMP+")";
	...
	public void perfectWorld() throws Exception{
		...
		// Make a payment using the payment link on the XSite order
		secure.makePaymentFromXSiteUsingPaymentLink(driver, orderNumber, setOrderFee, setTechFee, setCombined, "Automation", "Test", "5555555555554444", "November", "2026", "73099", "automation@dntest.net");
		...
		String serviceFeePercent = mercuryOrderReceivables.get(1);
		
		// Set expected ServiceFeePercent
	...
	public void secondaryCollection() throws Exception{
		...
		// Verify Order Summary payment details
		receivable = setNewCombined;
		received = setCombined;
		receivableDue = "$" + Integer.toString(receivableDueInt) + ".00";
		payable = setNewCombined;
		payableDue = setNewCombined;
		balance = setCombined;
		secure.verifyOrderSummaryPaymentDetailsInPayments(driver, ordered, loanNumber, propertyAddress, borrower, secureUser, vendorsUser, receivable, received, receivableDue, payable, payableDue, issueDate, toVendor, techFee, balance);
		...
		if (env.equals("QA") && runVPP==true) {
			...
			// Verify Order Summary payment details
			receivable = setNewCombined;
			received = setNewCombined;
			receivableDue = "$0.00";
			payable = setNewCombined;
			payableDue = setNewCombined; // payableDue = "$0.00";
			toVendor = "$0.00"; // toVendor = "550.00";
			techFee = "$0.00"; // techFee = setTechFee;
			balance = setNewCombined; // balance = "$0.00";
			secure.verifyOrderSummaryPaymentDetailsInPayments(driver, ordered, loanNumber, propertyAddress, borrower, secureUser, vendorsUser, receivable, received, receivableDue, payable, payableDue, issueDate, toVendor, techFee, balance);
			...
			amountDue = "$0.00";
			type = "Receivable";
			received = setCombined; // this should be removed because the value it should be is set prior to this
			secure.verifyInvoicesPaymentDetailsInPayments(driver, invoiceDate, loanNumber, propertyAddress, vmpUser, company, orderNumberVMP, invoiceDue, type, amount, received, paid, amountDue, SPayments.gridInvoices_txt());
			...
			perform.verification(driver, receivedText, "contains", setCombined.replace(".00", ""));
	...
	public void vendorWithNoACH() throws Exception{
		...
		payableDue = setCombined;
		waitForPayableDueToUpdate(driver, loanNumber, setCombined);
		
		// Verify Order Summary payment details
		receivable = setCombined;
		received = setCombined;
		receivableDue = "$0.00";
		payable = setCombined;
		issueDate = StoredVariables.gettodaysDateLong().get();
		...
		if (env.equals("QA") && runVPP==true) {
			...
			// Verify Order Summary payment details
			receivable = setCombined;
			received = setCombined; // is setCombined
